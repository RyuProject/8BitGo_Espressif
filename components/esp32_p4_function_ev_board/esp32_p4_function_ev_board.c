...
/*
 * ESP32-P4 只有 **1 个** SDMMC 控制器（SDMMC_LL_HOST_CTLR_NUMS = 1U），
 * 两个 slot 共用它：slot1 = 板载 ESP32-C6 的 SDIO 链路（esp-hosted），
 * slot0 = TF 卡。esp-hosted 启动时已经调用过 sdmmc_host_init() 把控制器
 * 创建并占用了，如果这里再调一次，就会撞上：
 *     sd_host_create_sdmmc_controller: no available sd host controller
 *     sdmmc_host_init: failed to create new SD controller
 * 官方 esp-hosted 示例（examples/mcu_hosted_sdio_sdmmc_combined）的做法是
 * 把 host.init/deinit 换成空实现，直接复用已存在的控制器、只配置 slot0。
 */
static esp_err_t bsp_sdmmc_host_init_dummy(void)
{
    return ESP_OK;
}

static esp_err_t bsp_sdmmc_host_deinit_dummy(void)
{
    return ESP_OK;
}
...
esp_err_t bsp_sdcard_mount(void)
{
    ...
    /* 控制器已经由 esp-hosted 持有：不要再 init/deinit */
    host.init = &bsp_sdmmc_host_init_dummy;
    host.deinit = &bsp_sdmmc_host_deinit_dummy;
#endif
...
#else
            /* 1280x800 竖屏（本硬件实测值）：
               X 轴需要镜像、Y 轴不需要；两轴均不交换。
               若你的屏幕走线不同，改这三个标志即可，无需动其它代码。 */
            .mirror_x = 1,
            .mirror_y = 0,
#endif
        },
