...
/* 置 1 可启用 DPI 的 DMA2D 加速；当前硬件/IDF 组合下默认关闭 */
#ifndef JD9365_ENABLE_DMA2D
#define JD9365_ENABLE_DMA2D 0
#endif
 ...
 static esp_err_t panel_jd9365_init(esp_lcd_panel_t *panel)
 {
     ...
     #if JD9365_ENABLE_DMA2D
         /* DMA2D 必须在 DPI 面板 init 之后启用。
            注意：在 ESP32-P4 rev1.0 + ESP-IDF v6.0 上启用后会持续报
            "previous draw operation is not finished"，故默认关闭。 */
