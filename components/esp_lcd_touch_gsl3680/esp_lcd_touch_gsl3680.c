...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    if (gsl3680_config && esp_lcd_touch_gsl3680->config.rst_gpio_num != GPIO_NUM_NC && esp_lcd_touch_gsl3680->config.int_gpio_num != GPIO_NUM_NC) {
        ...
        /* 这条分支是「未同时提供 INT/RST 引脚（或 driver_data）」时的正常路径：
           跳过 I2C 地址选择时序，只做一次软复位。原日志文案会让人误以为出错。 */
        ESP_LOGI(TAG, "skip I2C addr-select (no int/rst pins), do soft reset");
...
{
    esp_err_t err;
    /* 下面从 0x80 一次读 44 字节，原来只声明 24 字节 —— 每次触摸轮询都会
       往栈上多写 20 字节，踩坏局部变量/栈保护字，必须留够。 */
    uint8_t touch_data[64] = {0};
...
// #endif
    ...
    /* 状态字节一旦异常（读失败/残帧）会取出很大的手指数，直接用它当循环次数
       会越界写 cinfo.x[] 与 XY_Coordinate[]，这里先夹一次。 */
    ...
    /* 算法输出的手指数同样要夹在 MAX_FINGER_NUM 内（XY_Coordinate 只有 3 项） */
    if (Finger_num > MAX_FINGER_NUM) {
        Finger_num = MAX_FINGER_NUM;
    }
...
static esp_err_t esp_lcd_touch_gsl3680_read_ram_fw(esp_lcd_touch_handle_t tp)
{
    ...
    /* 0xb0 读到 5a5a5a5a 才是「正确」的（固件已跑起来）。
       原代码把这条日志写在判断之前、且文案写成 failed，很容易误判成触摸坏了。 */
    if (read_buf[3] != 0x5a || read_buf[2] != 0x5a || read_buf[1] != 0x5a || read_buf[0] != 0x5a) {
        ESP_LOGW(TAG, "ram fw check mismatch: 0xb0 = %02x%02x%02x%02x (expect 5a5a5a5a)",
                 read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "ram fw check ok (0xb0 = 5a5a5a5a)");
    return ESP_OK;
}
