...
#ifdef CONFIG_IDF_TARGET_ESP32P4
/* IRAM_ATTR：IDF 要求 on_color_trans_done 回调必须位于 IRAM，否则注册被拒，
   下方每 chunk 的 portMAX_DELAY 等待会永久阻塞 -> 首帧死锁 */
IRAM_ATTR static bool lcd_event_callback(esp_lcd_panel_handle_t panel_io, esp_lcd_dpi_panel_event_data_t *edata, void *user_ctx)
{
    xSemaphoreGive(lcd_semaphore);
    return false;
}
#else
IRAM_ATTR static bool lcd_event_callback(esp_lcd_panel_io_handle_t io, esp_lcd_panel_io_event_data_t *event_data, void *user_ctx)
{
    xSemaphoreGive(lcd_semaphore);
    return false;
}
